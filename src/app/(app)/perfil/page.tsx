...
export default function PerfilPage() {
  ...
  async function handleSavePhone() {
    ...
    else toast.success("Celular atualizado!");
  ...
  async function handleSaveNotif() {
    ...
    setSavingNotif(true);
    const supabase = createClient();
    const { error } = await supabase.from("profiles").update({ notif_email: notifEmail, notif_whatsapp: notifWhatsapp }).eq("id", userId);
  ...
  async function handleSaveAno() {
    ...
    if (error) toast.error("Erro ao salvar ano.");
      ...
      <div className="card space-y-5">
        ...
        <form onSubmit={handleSaveName} className="space-y-4">
          ...
          <div>
            ...
            <input type="email" className="input bg-gray-50 cursor-not-allowed" value={email} disabled />
          ...
          <button type="submit" disabled={savingName} className="btn-primary flex items-center gap-2">
            {savingName ? <><Loader2 className="w-4 h-4 animate-spin" /> Salvando...</> : "Salvar nome"}
          </button>
        </form>
        ...
        <div className="border-t border-gray-100 pt-5">
          ...
          <div className="flex gap-2">
            ...
            />
            <button type="button" onClick={handleSaveCnpj} disabled={savingCnpj} className="btn-secondary text-sm whitespace-nowrap flex items-center gap-1.5">
              {savingCnpj ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : null}
              Salvar
            </button>
          </div>
        ...
        <div className="card space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={handleSavePhone} disabled={savingPhone} className="btn-secondary text-sm whitespace-nowrap flex items-center gap-1.5">
                {savingPhone ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : null}
                Salvar
              </button>
            </div>
            <p className="text-xs text-gray-400 mt-1">Você receberá alertas quando se aproximar do limite anual de R$ 81.000.</p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3">
          <label className="flex items-center justify-between gap-3 cursor-pointer">
            <div>
              ...
              <p className="text-xs text-gray-400">Aviso quando atingir 70%, 90% e 100% do limite</p>
            </div>
            <button
              type="button"
              onClick={() => setNotifEmail(!notifEmail)}
              className={`relative w-10 h-6 rounded-full transition-colors ${notifEmail ? "bg-brand-500" : "bg-gray-200"}`}
            >
          ...
          {isPremium && (
            <label className="flex items-center justify-between gap-3 cursor-pointer">
              <div>
                ...
                <p className="text-xs text-gray-400">Mensagem no celular cadastrado acima</p>
              </div>
              <button
                type="button"
                onClick={() => setNotifWhatsapp(!notifWhatsapp)}
                className={`relative w-10 h-6 rounded-full transition-colors ${notifWhatsapp ? "bg-purple-500" : "bg-gray-200"}`}
              >
      ...
      <div className="card space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={handleSaveAno} disabled={savingAno} className="btn-secondary text-sm whitespace-nowrap flex items-center gap-1.5">
              {savingAno ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : null}
              Salvar
            </button>
          </div>
          <p className="text-xs text-gray-400 mt-1">O dashboard mostra o faturamento do ano selecionado.</p>
        </div>
      </div>
      ...
      <div className="card space-y-4">
        ...
        <form onSubmit={handleSavePassword} className="space-y-4">
          ...
          <button type="submit" disabled={savingPassword} className="btn-primary flex items-center gap-2">
            {savingPassword ? <><Loader2 className="w-4 h-4 animate-spin" /> Salvando...</> : "Atualizar senha"}
          </button>
        </form>
      </div>
    </div>
